...
local function atmo_winter_enterframe(_, speed)
	...
	for i = 0, nb_flakes do
		...
		flakes_pos_y[i] = flakes_pos_y[i] + flakes_amp_y[i] * math.cos(flakes_pos_z[i]) * speed - flakes_speed[i] * speed
		flakes_pos_x[i] = flakes_pos_x[i] + flakes_amp_x[i] * math.cos(flakes_pos_x[i]) * speed - 2 * flakes_speed[i] * speed
