...
local function setModSettings()
	...
	UIElement:runCmd("set dismemberthreshold 150")
	start_new_game()
end
...
local function eventMain(viewElement, reqTable, skipAdd)
	...
	if (skipAdd == 0) then
		...
		add_hook("key_up", "tbTutorialsCustom", function(key)
				...
				if (key == 101) then
					...
					tbTutorialsTask.frames = 0
					tbTutorialsTask.maxFrames = 0
					tbTutorialsTaskMark:hide(true)
	...
	add_hook("draw2d", "tbTutorialsCustom", function()
			...
			if (ws.match_frame ~= frame_checked) then
				local height = 100
				for i = 0, 19 do
					local p = get_joint_pos2(1, i)
					local s = get_joint_radius(1, i)
					if (p.z - s < height) then
						height = p.z - s
					end
				end
				for i = 0, 20 do
					local body = get_body_info(1, i)
					if (body.pos.z < height) then
						height = body.pos.z
					end
				end
				height = math.ceil(height * 100 - 0.5) / 100
				if (height >= 0.35) then
					tbTutorialsTask.frames = tbTutorialsTask.frames + 1
					tbTutorialsTask.maxFrames = tbTutorialsTask.frames
				else
					tbTutorialsTask.frames = 0
				end
				...
				if (#tbTutorialsTask.extra > 0) then
					...
					textView:addAdaptedText(true, displayText .. string.format("%d", tbTutorialsTask.frames) .. ' frames' .. (tbTutorialsTask.maxFrames > tbTutorialsTask.frames and (' (' .. string.format("%d", tbTutorialsTask.maxFrames) .. ' frames best)') or ''), nil, nil, 4, LEFTMID, 0.6)
				end
				
				if (not tbTutorialsTaskMark:isDisplayed()) then
					if (tbTutorialsTask.maxFrames >= 200) then
				...
				if (tbTutorialsTask.optional[1] and not tbTutorialsTask.optional[1].complete) then
					if (tbTutorialsTask.maxFrames >= 650) then
