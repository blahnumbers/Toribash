...
local finalPos = { x = 3.3, y = 8.3, z = 0.25, rad = 3, shape = CUBE, size = { 3.20, 2.20, 5 }, complete = tbTutorialsTaskMark and tbTutorialsTaskMark:isDisplayed(), visible = false }
--local trigObjectId = 32
local eggIds = { 15, 14, 13, 12, 9, 8, 7, 6, 3, 2 }
local checkPoints = {
...
local function showUploadWindow(viewElement, reqTable)
	...
	local function uploadReplay(name)
		...
		if (file.data) then
			local winframe = 100000 + WIN_FRAME
			for i,v in pairs(eggIds) do
				...
				if (not finalPos.complete) then
					...
					for i,v in pairs(eggIds) do
						...
						if (finalPos.x - finalPos.size[1] / 2 < x and finalPos.x + finalPos.size[1] / 2 > x and
							...
							finalPos.z - finalPos.size[3] / 2 < z and finalPos.z + finalPos.size[3] / 2 > z) then
							REPLAY_CAN_BE_SUBMITTED = true
							eggsInLocal = eggsInLocal + 1
						end
					end
					if (eggsInLocal ~= eggsIn) then
						if (eggsIn < eggsInLocal) then
							WIN_FRAME = ws.match_frame
						end
						eggsIn = eggsInLocal
					end
