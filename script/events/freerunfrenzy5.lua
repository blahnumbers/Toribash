...
local finalPos = { x = 0, y = 34.4, z = 0, rad = 2, shape = CAPSULE, complete = tbTutorialsTaskMark and tbTutorialsTaskMark:isDisplayed(), visible = true }
local checkPoints = {
	{ x = 0.5, y = 23.9, z = 9, h = 3.49, w = 3.49, zh = 3.5, shape = CUBE, color = { 0, 0, 1, 0.5 }, task = 1, complete = tbTutorialsTask.optional[1] and tbTutorialsTask.optional[1].complete, costModifier = 1 }
}
...
local NUM_JOINTS = 20
local MIN_CHECKPOINT_JOINTS = 16
local NUM_JOINTS_OUTSIDE_CHECKPOINT = math.max(0, NUM_JOINTS - MIN_CHECKPOINT_JOINTS)

local SHOW_FRAMES_ELAPSED = false
local SHOW_TOPLIST = false

local function spawnToplist(viewElement)
	if (not SHOW_TOPLIST) then return end
	...
	local toplistSuccess = function()
		...
		targetFrames:addCustomDisplay(false, function()
				...
				targetFrames:uiText("Top speed: " .. topSpeed .. " frames", -65, 100, FONTS.MEDIUM, nil, 0.8, -90, nil, { 1, 1, 1, targetFrames.bgColor[4] })
			end)
	end
...
local function showUploadWindow(viewElement, reqTable)
	...
	local function uploadReplay(name)
		...
		if (Request.queue ~= nil) then
			Request:queue(function()
				upload_event_replay(name, "Event Squad's Free Run Frenzy 5 entry", "ESEVNT" .. CURRENT_TUTORIAL, "replay/my replays/" .. name .. ".rpl")
...
local function eventMain(viewElement, reqTable, skipAdd)
	...
	if (SHOW_FRAMES_ELAPSED) then
		...
		framesElapsed:addCustomDisplay(true, function()
				...
				if (ws.match_frame > 0 or WIN_FRAME < 100000) then
					local frame = WIN_FRAME < 100000 and WIN_FRAME or ws.match_frame
	...
	add_hook("draw2d", "tbTutorialsCustom", function()
			...
			if (ws.match_frame >= ws.game_frame + 40) then
				TUTORIAL_LEAVEGAME = true
				rewind_replay()
				TUTORIAL_LEAVEGAME = false
				return
...
local function setDiscordRPC()
	...
	set_discord_rpc("Free Run Frenzy " .. tutorialNum, TB_MENU_LOCALIZED.DISCORDRPCPLAYINGSPEVENT or "Playing SP Event")
end
