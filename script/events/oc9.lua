...
local airtimeUpdate = function(_, check, target)
	local ws = get_world_state()
	if (tbTutorialsTask.misc[check.task] == nil) then
		tbTutorialsTask.misc[check.task] = { }
	end

	local v = tbTutorialsTask.misc[check.task].last
	if (v ~= nil) then
		if (v.frame_end == ws.match_frame) then return false end
		if (v.frame_end == ws.match_frame - 1) then
			v.frame_end = ws.match_frame
			return v.frame_end - v.frame_start >= target
		end
	end

	table.insert(tbTutorialsTask.misc[check.task], {
		frame_start = ws.match_frame, frame_end = ws.match_frame
	})
...
local airtimeReset = function(check, frame, target)
	if (tbTutorialsTask.misc[check.task] == nil) then return end
	local lastID = #tbTutorialsTask.misc[check.task]
	for i = lastID, 1, -1 do
		if (tbTutorialsTask.misc[check.task][i].frame_start > frame) then
			lastID = lastID - 1
			table.remove(tbTutorialsTask.misc[check.task], i)
			tbTutorialsTask.misc[check.task].last = tbTutorialsTask.misc[check.task][lastID]
		elseif (tbTutorialsTask.misc[check.task][i].frame_end > frame) then
			tbTutorialsTask.misc[check.task][i].frame_end = frame
		end
	end
	if (tbTutorialsTask.misc[check.task].last == nil or tbTutorialsTask.misc[check.task].last.frame_end - tbTutorialsTask.misc[check.task].last.frame_start < target) then
		return true
	end
	return false
end
	...
	GLOBAL_SETTINGS = {
		...
		ON_FRAME_CHECK_CALLBACK = function(checkPoints)
			if (EventsOnline.TaskView ~= nil and EventsOnline.LocalizedMessages ~= nil) then
				...
				if (last ~= nil and last.frame_end == EventsOnline.WorldState.match_frame) then
					msg = utf8.gsub(EventsOnline.LocalizedMessages.EVTTASK1VAL, "{x}", last.frame_end - last.frame_start)
				end
	...
	CHECKPOINTS = {
		{
			...
			task = 4,
			pos = { x = 0, y = 0, z = 50 },
			size = { x = 1000, y = 1000, z = 99 },
			shape = CUBE,
			color = { 1, 0, 0, 0.4 },
			targetCheck = CHECK_TYPE.PLAYER_POS,
			playerId = 1,
			checkFunction = function(_, check) return airtimeUpdate(_, check, 500) end,
			invalidateFunc = function(check, frame) return airtimeReset(check, frame, 500) end
		},
		{
			visible = false,
			costModifier = 2,
		...
		},
		{
			...
			targetCheck = CHECK_TYPE.PLAYER_POS,
			playerId = 1,
			checkFunction = function(_, check) return airtimeUpdate(_, check, 150) end,
			invalidateFunc = function(check, frame) return airtimeReset(check, frame, 150) end
		},
		{
			visible = false,
			costModifier = 1,
		...
		},
		{
			...
			checkFunction = function()
				return get_grip_lock(0, BODYPARTS.L_HAND) ~= -1 or get_grip_lock(0, BODYPARTS.R_HAND) ~= -1
			end,
		}
	}
}
