...
local function resetOrbs()
	if (not ORBS_DATA) then
		return
	end
	for i,v in pairs(ORBS_DATA) do
		v.touched = false
		if (v.color) then
			set_obj_color(v.id, unpack(v.color))
		end
	end
end
...
local function showUploadWindow(viewElement, reqTable)
	...
	local function uploadReplay(name)
		...
		if (file.data) then
			...
			for i,v in pairs(ORBS_DATA) do
				if (v.touched) then
					winframe = winframe - (v.cost and v.cost or 100)
				end
			end
		...
		if (Request.queue ~= nil) then
			Request:queue(function()
				upload_event_replay(name, "Event Squad's Flash Course 2 event entry", "ESEVNT" .. CURRENT_TUTORIAL, "replay/my replays/" .. name .. ".rpl")
...
local function eventMain(viewElement, reqTable, skipAdd)
	...
	framesElapsed:addCustomDisplay(true, function()
			...
			if (ws.match_frame > 0 or WIN_FRAME < 100000) then
				...
				if (WIN_FRAME >= 100000) then
					frame = ws.match_frame
					for i,v in pairs(ORBS_DATA) do
						if (v.touched) then
							frame = frame - (v.cost and v.cost or 100)
						end
					end
				end
...
local function launchGame(viewElement, reqTable)
	...
	ORBS_DATA = { { id = 0, cost = 250, color = { 1.4, 0.2, 0.5, 1 } }, { id = 1, cost = 75, color = { 0, 1.5, 0.6, 1 } }, { id = 2, cost = 150, color = { 0.3, 0, 1.5, 1 } } }
	for i,v in pairs(ORBS_DATA) do
		local x, y, z = get_obj_pos(v.id)
		ORBS_DATA[i].pos = { x, y, z }
	end
	resetOrbs()
end
