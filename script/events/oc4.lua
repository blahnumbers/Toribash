...
_T = {
	GLOBAL_SETTINGS = {
		...
		ON_FRAME_CHECK_CALLBACK = function(_)
			if (EventsOnline.TaskView ~= nil and EventsOnline.LocalizedMessages ~= nil) then
				local minHeight = 100000
				for _, v in pairs(JOINTS) do
					minHeight = math.min(minHeight, math.floor(get_joint_pos2(1, v).z * 10) / 10)
				end
				if (minHeight > 0.5) then
					local msg = utf8.gsub(EventsOnline.LocalizedMessages.EVTTASKDETAILED, "{x}", tostring(minHeight))
					...
					EventsOnline.TaskView:addAdaptedText(true, EventsOnline.LocalizedMessages.EVTTASK1, nil, nil, EventsOnline.TaskView.textFont, LEFTMID, EventsOnline.TaskView.textScale)
				end
			end
		end
	},
