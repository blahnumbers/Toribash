...
local function eventMain(viewElement, reqTable, skipAdd)
	...
	add_hook("draw2d", "tbTutorialsCustom", function()
			...
			if (ws.match_frame ~= frame_checked) then
				local dismemberedJoints = 0
				for i = 0, 19 do 
					if (get_joint_dismember(1, i)) then
						dismemberedJoints = dismemberedJoints + 1
					end
				end
				tbTutorialsTask.dismembers = dismemberedJoints
				
				if (#tbTutorialsTask.extra > 0) then
					local textView = tbTutorialsTask.extra[1].textView
					local displayText = textView.str:gsub(":.*$", ': ')
					textView:addAdaptedText(true, displayText .. string.format("%d", tbTutorialsTask.dismembers) .. ' DMs', nil, nil, 4, LEFTMID, 0.6)
				end
				
				if (not tbTutorialsTaskMark:isDisplayed()) then
					if (tbTutorialsTask.dismembers >= 3) then
				...
				if (tbTutorialsTask.optional[1] and not tbTutorialsTask.optional[1].complete) then
					if (tbTutorialsTask.dismembers >= 5) then
...
local function checkOpenerReplay(viewElement, reqTable)
	...
	local headTexture = Files:open("../custom/divine/head.tga")
