...
local function setModSettings()
	UIElement:runCmd("set mod system/events/openerch8.tbm")
	start_new_game()
	UIElement:runCmd("set matchframes 1500")
	UIElement:runCmd("set engagedistance 800")
	UIElement:runCmd("set gravity 0 0 -30")
	UIElement:runCmd("set engageplayerpos -10,-21,1,0,8,1")
	UIElement:runCmd("set flags 0")
	start_new_game()
end
...
local function eventMain(viewElement, reqTable, skipAdd)
	...
	if (skipAdd == 0) then
		...
		local floorLevelDraw = UIElement3D:new({
		...
		add_hook("key_up", "tbTutorialsCustom", function(key)
				...
				if (key == 32) then
					if (ws.replay_mode == 1) then
						local replayName = CURRENT_TUTORIAL .. "-" .. os.date("%d-%H-%M-%S")
						...
						STOPFRAME = openerStopframe
						start_new_game()
						loadExistingReplay(viewElement, reqTable, true, '') -- path not being empty is a quick hack to prevent from proceeding to next step
						REPLAY_CAN_BE_SUBMITTED = false
						TUTORIAL_LEAVEGAME = false
						TBMenu:showDataError("Replay has been autosaved to my replays folder as " .. replayName, true)
						return 1
		...
		add_hook("key_down", "tbTutorialsCustom", function(key)
				...
				--[[if (key == 118 and get_keyboard_ctrl() > 0) then
					if (floorLevelDraw:isDisplayed()) then
						floorLevelDraw:hide()
					else
						floorLevelDraw:show()
					end
	...
	local playerScore = UIElement:new({
		parent = viewElement,
		pos = { viewElement.size.w / 2, 2 },
		size = { viewElement.size.w / 2 - 5, 50 },
	...
	add_hook("draw2d", "tbTutorialsCustom", function()
			local points = get_player_info(1).injury
			points = math.floor(points + 0.5)
			playerScore:uiText(points, 0, 0, 0, RIGHT)
			...
			if (ws.match_frame >= ws.game_frame) then
				...
				rewind_replay()
				TUTORIAL_LEAVEGAME = false
				-- Only for opener challenge 8
				REPLAY_CAN_BE_SUBMITTED = true
				return
			end
			--[[if (ws.match_frame ~= frame_checked and ws.match_frame >= openerStopframe) then
				local frameMaxVelocity = 0
				for i = 0, 21 do
					local velocity = { get_body_linear_vel(0, i) }
					local magnitude = math.sqrt(velocity[1] * velocity[1] + velocity[2] * velocity[2] + velocity[3] * velocity[3])
					if (magnitude > frameMaxVelocity) then
						frameMaxVelocity = math.floor(magnitude * 100 + 0.5) / 100
					end
				end
				if (frameMaxVelocity > tbTutorialsTask.maxVelocity) then
					tbTutorialsTask.maxVelocity = frameMaxVelocity
				end
				
				if (#tbTutorialsTask.extra > 0) then
					local textView = tbTutorialsTask.extra[1].textView
					local displayText = textView.str:gsub(":.*$", ': ')
					textView:addAdaptedText(true, displayText .. tbTutorialsTask.maxVelocity .. ' (current ' .. frameMaxVelocity .. ')', nil, nil, 4, LEFTMID, 0.6)
				end
				
				if (not tbTutorialsTaskMark:isDisplayed()) then
					if (tbTutorialsTask.maxVelocity >= 40) then
						EventsOnline:taskComplete(true)
						REPLAY_CAN_BE_SUBMITTED = true
					end
				end
				if (tbTutorialsTask.optional[1] and not tbTutorialsTask.optional[1].complete) then
					if (tbTutorialsTask.maxVelocity >= 55) then
