...
function DiscordRPC:show()
	...
	local discordBackgroundAnimation = UIElement:new({
		...
		pos = { 0, 0 },
		size = { discordOverlay.size.w, discordOverlay.size.h }
	})
	local circles = {}
	while (#circles < 30) do
	...
	discordAccept:addCustomDisplay(false, function()
			discordAccept:uiText("Accept")
		end)
	discordAccept:addMouseHandlers(nil, function()
			DiscordRPC:quit(discordOverlay, buttons)
			discord_accept_join(DiscordUserid)
		end)
	discordDecline:addCustomDisplay(false, function()
			discordDecline:uiText("Decline")
		end)
...
function DiscordRPC:drawVisuals()
...
add_hook("draw2d", "tbDiscordRPCVisuals", function() DiscordRPC:drawVisuals() end)
