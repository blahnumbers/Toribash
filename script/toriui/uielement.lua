	...
	self:addMouseHandlers(
		function(s, x, y)
			...
			elseif (not UIScrollbarIgnore and ((#UIScrollbarHandler == 1 and listHolder.scrollBar ~= self) or
				...
				local scroll = self.orientation == SCROLL_HORIZONTAL and (math.abs(x) > math.abs(y) and -x or y) or y
				self:mouseScroll(listElements, listHolder, toReload, scroll * scrollSpeed, enabled)
...
---@param listElements UIElement[]
---@param listHolder UIElement
---@param toReload UIElement deprecated
---@param scroll number
---@param enabled UIElement[] deprecated
