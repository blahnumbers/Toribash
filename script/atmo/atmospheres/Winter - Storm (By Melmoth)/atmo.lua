...
local nb_flakes = 260 -- if TB crashes when you load this atmospheres, try to reduce this value
...
for i=0,nb_flakes do
	flakes_pos_x[i] = math.random(-1100,1100)/100
	flakes_pos_y[i] = math.random(-1100,1100)/100
	flakes_pos_z[i] = math.random(0,1100)/100
	flakes_rad[i]   = math.random(20,50)/1000
	flakes_speed[i] = math.random(80,100)/1000
	flakes_amp_y[i] = math.random(1,10)/1000
	flakes_amp_x[i] = math.random(1,10)/1000	
end

local function atmo_snow_storm()
	if(get_option("shaders")==0) then
		set_color(2.00,2.00,2.00,1)
		draw_box(0, 0, 0.025, 1000, 1000, 0.08, 0, 0, 0)
		set_color(1,1,1,0.1)
		for i=1,30 do
		 draw_sphere(0, 0, 0, math.random(-100,100)/1000 + 2*i)
		end
	else
		camera_info = get_camera_info()
		set_color(-1.00, -1.00, -1.00,-0.2)
		for i=1,5 do
			draw_sphere(math.random(-100,100)/500 + camera_info.pos.x,math.random(-100,100)/500 + camera_info.pos.y,math.random(-100,100)/500 + camera_info.pos.z,-i)
		end
	end
	...
	for i=0,nb_flakes do
		...
		if(is_animated()) then
			...
			flakes_pos_y[i] = flakes_pos_y[i] + flakes_amp_y[i] * math.cos(flakes_pos_z[i]) - 2*flakes_speed[i]
			flakes_pos_x[i] = flakes_pos_x[i] + flakes_amp_x[i] * math.cos(flakes_pos_x[i]) - 2*flakes_speed[i]
			if(flakes_pos_z[i] < 0) then
				flakes_pos_z[i] = 11
			end
			if(flakes_pos_y[i] < -11) then
				flakes_pos_y[i] = 11
			end
			if(flakes_pos_x[i] < -11) then
				flakes_pos_x[i] = 11
			end
		end
	end
...
add_hook("draw3d", "atmo", atmo_snow_storm)