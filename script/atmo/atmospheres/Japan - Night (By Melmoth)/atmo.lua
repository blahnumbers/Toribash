...
local nb_leaves = 55 -- if TB crashes when you load this atmospheres, try to reduce this value
local nb_leaves_floor = 15 -- if TB crashes when you load this atmospheres, try to reduce this value
...
local function atmo_japan_daylight()

	if(get_option("shaders")==0) then
		set_color(0.00, 0.40, 0.00,1)
		draw_box(0, 0, 0.025, 1000, 1000, 0.08, 0, 0, 0)
		set_color(0.10, 0.10, 0.60,1)
	...
	for i=0,nb_leaves do
		...
		if(is_animated()) then
			leaves_pos_z[i] = leaves_pos_z[i] - leaves_speedz[i]
			leaves_pos_x[i] = leaves_pos_x[i] + leaves_speedx[i]
			leaves_pos_y[i] = leaves_pos_y[i] + leaves_speedy[i]
			
			leaves_rot_x[i] = leaves_rot_x[i] + leaves_drot_x[i]
			leaves_rot_y[i] = leaves_rot_y[i] + leaves_drot_y[i]
			leaves_rot_z[i] = leaves_rot_z[i] + leaves_drot_z[i]
...
add_hook("draw3d", "atmo", atmo_japan_daylight)