...
function draw_hand_tooltip()
	if (hand_text ~= nil and hand_joint_render == true) then
		...
		if (get_grip_info(hand_text.player, hand_text.hand) == 0) then
			hand_text['screen_state'] = "SOLTAR"
		else
			hand_text['screen_state'] = "SEGURAR"
		end
		if (hand_text.body_name == "R_HAND") then
			hand_text.body_name = "MÃO DIREITA"
		elseif (hand_text.body_name == "L_HAND") then
			hand_text.body_name = "MÃO ESQUERDA"
		end
...
function draw_joint_tooltip()
	if (joint_text ~= nil and hand_joint_render == true) then
		...
		-- Draw joint name
		set_color(0.5, 0.5, 0.5, 0.8)
		draw_quad(joint_text.x + 30, joint_text.y + 10, 240, 30)
		set_color(0.0, 0.0, 0.0, 1.0)
		draw_text(joint_text.joint_name, joint_text.x + 40, joint_text.y + 15)

		-- Draw joint state
		set_color(0.7, 0.7, 0.7, 0.8)
		draw_quad(joint_text.x + 30, joint_text.y + 40, 240, 30)
		set_color(0.0, 0.3, 0.0, 1.0)
...
function render_quad_alert(count)
	if (quad_alert_render == true) then
		...
		if (count == 2) then
			draw_quad(win_w - 110 - quad_alert_size, 165 - quad_alert_size, 65 + quad_alert_size*2, 110 + quad_alert_size*2)
		elseif (count == 3) then
			draw_quad(win_w - 110 - quad_alert_size, 90 - quad_alert_size, 65 + quad_alert_size*2, 55 + quad_alert_size*2)
		end
	end
end
...
function draw_spacebar()
	...
	draw_text("SPACE", win_w/2 - 300, win_h - (win_h/4) - 80, FONTS.BIG)
end
function draw_shift()
...
function draw_z()
	...
	draw_text("W", win_w/2 - 310, win_h/2 + 60, FONTS.BIG)
end
function draw_x()
	set_color(0.5, 0.5, 0.5, 0.8)
...
function draw_mpqueue()
	...
	if (mpqueue == 2) then
		draw_quad(win_w - 110, 165, 65, 110)	-- Draw box around waiting list
	elseif (mpqueue == 3) then
		draw_quad(win_w - 110, 90, 65, 55)		-- Draw box around playing list
	end
	...
	set_color(0.0, 0.0, 0.0, 1.0)
	draw_text("sir", win_w - 100, 170)
	draw_text("Dranix", win_w - 100, 220)
...
local intro_msg = {
	"Bienvenue sur le tutoriel de Toribash!",
 }
local camera_msg = {
	"Contrôles de la caméra!",
	"Appuyez sur 'Z/Q/S/D' pour bouger la caméra",
	"Maintenez Maj en appuyant sur 'Z/S'",
	"pour changer la hauteur de la caméra",
}
local controls_msg = {
	"Bouger votre Tori!",
	"Faites un clic gauche sur les articulations",
	"ou utilisez la roulette pour les faire changer d'état!",
	"Faites un clic gauche sur les mains pour qu'elles", 
	"aggripent l'adversaire lorsqu'elles le touchent!",
	"Quand votre curseur passe au dessus d'une articulation",
	"vous pouvez aussi appuyer sur 'W' ou 'X'",
	"Appuyez sur 'C' pour faire passer toutes les articulations",
	"en 'Maintenu' ou en 'Relaxé'",
	"Appuyez sur la barre d'espace pour terminer votre tour",
}
local game_msg = {
	"Vous pourrez réaliser d'incroyables mouvements",
	"en combinant le mouvement de plusieurs articulations!",
	"Vous voyez ce ring par terre?",
	"C'est un ring de disqualification (DQ)",
	"Vous serez disqualifié si votre corps",
	"touche par terre en premier!",
	"Disqualifier vos adversaires",
	"or faites un plus haut score qu'eux pour gagner!",
}
local multiplay_msg = {
	"Jouer en ligne!",
	"Après être entré dans une salle,",
	"vous serez mis dans la liste d'attente",
	"Les joueurs en train de combattre",
	"sont affichés ici",
	"Le gagnant du match combattra avec",
	"le prochain joueur sur la liste",
	"Une fois que vous aurez maîtrisé quelques mouvements,",
	"testez votre niveau contre d'autres joueurs!",
}
