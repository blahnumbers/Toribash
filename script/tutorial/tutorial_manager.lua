...
if (Tutorials == nil) then
	...
	---* New `Tutorials.LoadFile()` method to make sure we can load files on mobile platforms
	---@class Tutorials
	---@field TooltipOverride boolean Whether tutorials tooltip override is enabled
	...
	if (get_option("tooltip") == 0 or (Tooltip == nil or not Tooltip.IsActive)) then
		require("system.tooltip_manager")
		Tutorials.TooltipOverride = true
	end
end
...
---@param path string
---@return function?
function Tutorials.LoadFile(path)
...
function Tutorials:loadTutorial(id, path)
	...
	for i, ln in pairs(tutorialData) do
		...
		elseif (ln:find("^LOADPLAYER")) then
			...
			local playerid = tonumber(ln:gsub("%D", "")) + 0
		...
		elseif (ln:find("^CUSTOMFUNC")) then
			...
			steps[#steps].customfuncfile = Tutorials.LoadFile(cfuncpath .. id .. ".lua")
...
function Tutorials:playFrames(viewElement, reqTable, frames)
	...
	if (Tutorials.ReplayCache == true) then
		set_replay_speed(Tutorials.ReplaySpeed)
	else
		frames = frames - 1
...
function Tutorials:loadReplay(viewElement, reqTable, replay, cache)
	...
	TUTORIAL_LEAVEGAME = true
	Tutorials.ReplayCache = cache ~= 0
	open_replay("system/tutorial/" .. replay, cache)
	if (Tutorials.ReplayCache) then
		set_replay_speed(0)
	end
	freeze_game()
