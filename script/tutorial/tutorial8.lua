...
function render_next_btn()
	...
	percentage = string.format("%i%s", math.floor(cal), "% done")
	draw_text(percentage, px, py)

	-- Draw the button text
	set_color(0, 0, 0, 0.3)
	if (percentage == "100% done") then
		draw_text("Fight Uke", next_btn.tx, next_btn.ty)
	else
		draw_text("Continue", next_btn.tx, next_btn.ty)
	end
end
...
function render_stop_btn()
	...
	-- Draw the button text
	set_color(0, 0, 0, 0.3)
	draw_text("End Tut", stop_btn.tx, stop_btn.ty)
end
...
local intro_msg = {
	"--------",
	"Advanced Moves  Tutorial",
	"--------",
	"Learn more fighting techniques,",
	"or challenge Uke if you are ready!",
}
local bodybreaker_msg = {
	"Technique  :  Body Breaker",
	"Grab your opponent by the head",
	"and smash his body apart!",
	"Follow these instructions",
	"to replicate the move",
	"Hold all your joints by pressing 'C',",
	"extend both ankles,",
	"then end the turn",
	"Contract both pecs,",
...
local threepiece_msg = {
	...
	"Grab your opponent by the arms",
