...
function draw_arrow_up()
	...
	draw_text("W", win_w/2 - 25, win_h/4 - 30, FONTS.BIG)
end
...
local intro_msg = {
	"Bem Vindo ao Tutorial de Toribash!",
 }
local camera_msg = {
	"Controles de Câmera!",
	"Pressione 'W/A/S/D' para mover a câmera",
	"Segure Shift e pressione 'W/S'",
	"para mudar a altura da câmera",
}
local controls_msg = {
	"Movendo seu Tori!",
	"Clique nas articulações ou use o botão de rolagem do mouse",
	"para alternar os movimentos!",
	"Clique nas mãos para segurar/soltar!",
	"Passando o cursor em cima de uma articulação",
	"você pode também pressionar 'Z' ou 'X'",
	"Pressione 'C' para alternar todas as articulações",
	"entre 'Forçar' e 'Relaxar'",
	"Pressione Espaço para terminar seu turno",
}
local game_msg = {
	"Você pode realizar movimentos incríveis",
	"combinando os estados das articulações!",
	"Está vendo aquele ringue no chão?",
	"É o ringue de desqualificação (DQ)",
	"Você será desqualificado se seu corpo",
	"tocar o chão primeiro!",
	"Desqualifique seus oponentes",
	"ou faça mais pontos que eles para vencer!",
}
local multiplay_msg = {
	"Modo Online!",
	"Ao entrar em uma sala",
	"você será posto em uma lista de espera",
	"Os jogadores que estão lutando",
	"são mostrados aqui",
	"O vencedor da partida enfrentará",
	"o próximo jogador da lista",
	"Quando você estiver dominando alguns movimentos,",
	"teste suas habilidades contra outros jogadores!",
}
...
function controls_4()
	...
	set_message(controls_msg[5], controls_msg[6])
...
function controls_5()
	set_message(controls_msg[7], controls_msg[8])
