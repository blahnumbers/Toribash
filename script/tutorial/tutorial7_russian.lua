...
function mouse_up(mouse_btn, x, y)
	...
	if (stop_btn.enabled == true) then
		...
		if (check_stop_btn(x, y) == true) then
			terminate()
			echo("Загляните на русскоязычный раздел форума Toribash!")
...
local intro_msg = {
	"--------",
	"Базовое обучение Toribash",
	"--------",
	"В этом обучении мы тебе покажем",
	"пару приёмов для драки!",
}
local part_one_msg = {
	"Итак, пришло время",
	"ударить твоего оппонента!",
	"Нажми 'C' для напряжения всех суставов,",
	"затем нажми на своё левое колено, чтобы разогнуть его",
	"Наконец, нажми пробел,",
	"чтобы выполнить движение!",
	"Классный удар!",
}
local part_two_msg = {
	"Давай теперь попробуем другую атаку",
	"В этот раз, разогни обе ноги,",
	"чтобы пнуть своего оппонента",
	"Затем нажми пробел,",
	"чтобы закончить ход",
	"Теперь, нажми на плечи, чтобы опустить обе руки,",
	"и отпусти соперника, убрав граб с ладоней",
	"И опять нажми пробел,",
	"чтобы закончить движение",
	"Тачдаун!",
